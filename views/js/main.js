...
var resizePizzas = function(size) {
    ...
    //Simplify the size changer by assigning a simple percentage to the three sizes
    ...
    //Carculate the width for all the pizza container
    var newWidth = sizeSwitcher(size);
    // Iterates through pizza elements on the page and changes their widths
    //Simplify the code to insert the new percentage to all pizza cintainer
...
function updatePositions() {
    ...
    //Carculate "phase" for each of the five possible positioning background pizza
    //create phase array
    var phase = [];
    for (i = 0; i < 5; i++) {
        phase[i] = Math.sin((document.body.scrollTop / 1250) + i);
    }
    //Invoke layout calculation phase in the following style loop
    ...
    //This makes the function automatically update everytime an animation frame is completed
...
document.addEventListener('DOMContentLoaded', function() {
    var cols = 8;
    var s = 256;
    //Reduce the number of wiggling pizzas from 200 to a more reseanable number since
    //the majority are non-visible off screen
    for (var i = 0; i < 40; i++) {
